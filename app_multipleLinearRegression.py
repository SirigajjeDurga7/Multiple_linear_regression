...
st.markdown("""
    <div class="card">
        ...
        <p>Predicts <b>Tip Amount</b> from <b>Total Bill and Group Size</b> using Multiple Linear Regression</p>               
    </div>
...
# visualization
st.markdown('<div class="card">',unsafe_allow_html=True)
st.subheader("Total Bill vs Tip Amount(with Size as another feature-multiple linear regression)")
fig,ax=plt.subplots()
...
st.markdown(f"""
    ...
    <p>
    Tip Depends on both <b>Total Bill</b> and <b>Number of people</b>.
    </p>
    </div>
    """,unsafe_allow_html=True)
...
group=st.slider(
    "Number of people :",df["size"].min(),df["size"].max(),3)
input_scaled=scaler.transform([[bill,group]])
